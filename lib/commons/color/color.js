 ...
 * @param {number} alpha
 */
// Note: the class deliberately uses underscore-prefixed properties rather
// than #private fields. The build transpiles private fields to WeakMap
// lookups, which made every channel access an order of magnitude slower —
// significant since Color is used heavily by color-contrast checks.
export default class Color {
  // _r, _g, _b: color channel values typically in the range of 0-1
  // (can go below or above)
  // _red, _green, _blue: color component values resolved to the sRGB
  // color space (0-255)
  ...
  parseString(colorString) {
    // pages tend to reuse a small set of distinct colors while color
    // parsing and gamut conversion in colorjs are expensive, so cache
    // the resolved channel values per color string
    const parsedColorCache = cache.get('color.parsedStrings', () => new Map());
    const cachedColor = parsedColorCache.get(colorString);
    if (cachedColor) {
      this.r = cachedColor.r;
      this.g = cachedColor.g;
      this.b = cachedColor.b;
      this.alpha = cachedColor.alpha;
      return this;
    }

    // Colorjs <v0.5.0 does not support rad or turn angle values
    // @see https://github.com/LeaVerou/color.js/issues/311
    const originalColorString = colorString;
    ...
    try {
      ...
      parsedColorCache.set(originalColorString, {
        r: this.r,
        g: this.g,
        b: this.b,
        alpha: this.alpha
