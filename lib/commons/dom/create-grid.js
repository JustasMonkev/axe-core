...
function createStackingOrder(vNode, parentVNode, treeOrder) {
  ...
  // compute once; isStackingContext reads many computed style
  // properties and is also needed by getStackLevel
  const vNodeIsStackingContext = isStackingContext(vNode, parentVNode);
  ...
  const stackLevel = getStackLevel(vNode, parentVNode, vNodeIsStackingContext);
  if (stackLevel !== null) {
 ...
 * @param {Boolean} vNodeIsStackingContext - Precomputed isStackingContext(vNode, parentVNode)
 * @return {Number|null}
 */
function getStackLevel(vNode, parentVNode, vNodeIsStackingContext) {
