...
export function convertSelector(selector) {
  // Selectors mostly come from a fixed set of rule and check definitions.
  // Parsing them is expensive when repeated for every node, so cache the
  // converted expressions for the duration of the run.
  const convertedSelectors = cache.get('convertedSelectors', () => new Map());
  if (convertedSelectors.has(selector)) {
    return convertedSelectors.get(selector);
  }

  let expressions = cssParser.parse(selector);
  expressions = expressions.selectors ? expressions.selectors : [expressions];
  const converted = convertExpressions(expressions);
  convertedSelectors.set(selector, converted);
  return converted;
}
