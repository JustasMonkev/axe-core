...
import isUniqueId from './is-unique-id';
...
// the same URI is parsed for every element the attribute appears on
// (once for the page statistics and again for each generated selector)
const friendlyUriEnd = memoize(uri => getFriendlyUriEnd(uri));
...
function getAttributeNameValue(node, at) {
  ...
  if (name.indexOf('href') !== -1 || name.indexOf('src') !== -1) {
    const friendly = friendlyUriEnd(node.getAttribute(name));
    if (friendly) {
...
function getNthChildString(elm, selector) {
  ...
  // groups of similar siblings produce the same selector, so cache which
  // children match it to avoid running matchesSelector on every sibling
  // again for each of them
  const matchingChildren = getMatchingChildren(elm.parentNode, selector);
  const hasMatchingSiblings = matchingChildren.some(sibling => sibling !== elm);
...
const getMatchingChildren = memoize(
  function getMatchingChildrenMemoized(parent, selector) {
...
/**
 * Filter the similar elements down to those that still match the selector
 * now that `features` was added at the top of it.
 *
 * The selector is built purely from ' > ' (direct child) combinators, so a
 * candidate that matched the previous selector matches the new one exactly
 * when its ancestor `depth` levels up matches the added features. Checking
 * that ancestor against the short features selector (with the result cached
 * per ancestor, since candidates share ancestors) is much cheaper than
 * matching every candidate against the full selector.
 *
 * @param {Element[]} similar	 Elements that match the selector so far
 * @param {String} features		 The feature selector added at the top
 * @param {Element|null} uniqueIdElm	The element the features were created
 * from, when those features are an id selector unique within the root
 * @param {Number} depth			 Levels between the candidates and the features
 * @returns {Element[]}
 */
function filterSimilar(similar, features, uniqueIdElm, depth) {
  // a unique id selector can only match its own element
  if (uniqueIdElm) {
    return groupByNthAncestor(similar, depth).get(uniqueIdElm) ?? [];
  }

  return filterSimilarByFeatures(similar, features, depth);
}

/**
 * The non-id part of filterSimilar. Memoized so that groups of similar
 * elements, which filter the same candidate array by the same features,
 * share both the work and the resulting array (which in turn keeps
 * groupByNthAncestor shared for the next level up).
 * @param {Element[]} similar
 * @param {String} features
 * @param {Number} depth
 * @returns {Element[]}
 */
const filterSimilarByFeatures = memoize(
  function filterSimilarByFeaturesMemoized(similar, features, depth) {
    const result = [];
    groupByNthAncestor(similar, depth).forEach((candidates, ancestor) => {
      if (matchesSelector(ancestor, features)) {
...
/**
 * Group elements by their nth parentElement. Groups of similar elements
 * (e.g. list items) go through the same candidate filtering against the
 * same candidate arrays, so this is memoized to only walk the ancestor
 * chains once per group rather than once per element.
 * @param {Element[]} candidates
 * @param {Number} depth
 * @returns {Map<Element, Element[]>}
 */
const groupByNthAncestor = memoize(
  function groupByNthAncestorMemoized(candidates, depth) {
    ...
    for (const candidate of candidates) {
      ...
      if (group) {
        ...
        byAncestor.set(ancestor, [candidate]);
      }
    }
    return byAncestor;
  }
...
function generateSelector(elm, options, doc) {
  ...
  let selector;
  let similar;
  // number of ' > ' separated levels in the selector so far
  let depth = 0;
   ...
   */
  do {
    const idFeature = getElmId(elm);
    let features = idFeature;
    if (!features) {
    ...
    if (!similar || similar.length > constants.selectorSimilarFilterLimit) {
      // getElmId only returns ids unique within the root, so a selector
      // that is just that id is known to match only this element
      similar =
        idFeature && selector === idFeature
          ? [elm]
          : findSimilar(doc, selector);
    } else {
      similar = filterSimilar(similar, features, idFeature ? elm : null, depth);
    }
